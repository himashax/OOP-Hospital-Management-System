package Servlets;

import java.io.IOException;



import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;

import Model.MedicalRecord;
import Services.IMedicalRecordService;
import Services.MedicalRecordServiceimpl;

/**
 * Servlet implementation class GetMedicalRecord
 */
@WebServlet("/GetMedicalRecord")
public class GetMedicalRecord extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GetMedicalRecord() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		HttpSession session = request.getSession();
		
		String recordID = request.getParameter("recordID");
		
		session.setAttribute("recordID", recordID);
		IMedicalRecordService iMedRecord = new MedicalRecordServiceimpl();
		MedicalRecord medRecord = iMedRecord.getMedicalRecords(recordID);
		request.setAttribute("medRecord", medRecord);
		
		RequestDispatcher dispatcher = request.getRequestDispatcher("/UpdateMedicalRecord.jsp");
		dispatcher.forward(request, response);
		
	}

}
